...
const RestaurantList: React.FC<RestaurantListProps> = ({ restaurants, onHover }) => {
  return (
    <div className="space-y-4">
      {restaurants.map((restaurant) => (
        <div
          key={restaurant.id}
          className="bg-whitetheme dark:bg-darkthemeitems rounded-xl shadow-sm overflow-hidden transition-all duration-200 hover:shadow-md dark:hover:shadow-[0_4px_6px_-1px_rgba(255,255,255,0.05)]"
          ...
          <Link to={`/restaurant/${restaurant.id}`} target="_blank" className="flex flex-col sm:flex-row">
            ...
            <div className="w-full sm:w-1/3 h-48 sm:h-auto relative">
              <img
                ...
                alt={restaurant.name}
                className="w-full h-full object-cover hover:scale-105 transition-transform duration-300 rounded-t-xl sm:rounded-t-none sm:rounded-l-xl"
              />
